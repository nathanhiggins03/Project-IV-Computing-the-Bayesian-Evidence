...
ggplot(df_all, aes(x = T, y = Estimate)) +
  ...
  labs(
    title = "Effect of the number of temperatures on the SMC estimator",
    x = "T",
    y = "Log Evidence"
  ) +
  theme_minimal()+
...
ggplot(df_efficiency, aes(x = mean_time, y = mc_sd)) +
  ...
  labs(
    title = "Effect of the number of temperatures on efficiency - SMC",
...
#RMSE vs mean runtime (SMC – varying number of temperatures)
...
ggplot(df_rmse, aes(x = mean_time, y = rmse)) +
  geom_point(size = 3) +
  geom_line() +
  geom_text(aes(label = T), vjust = -0.6, hjust = 0.9) +
  labs(
    title = "Effect of the number of temperatures on efficiency – SMC",
    x = "Mean runtime (seconds)",
